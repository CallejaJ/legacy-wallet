...
4. [Flujo EOA (Externally Owned Account)](#flujo-eoa)
5. [Flujo Smart Account (ERC-4337)](#flujo-smart-account)
6. [Arquitectura del EntryPoint](#arquitectura-del-entrypoint)
...
**ECDSA** (Elliptic Curve Digital Signature Algorithm) es el algoritmo de firma digital que usa Ethereum para verificar que eres el propietario de una cuenta.
...
Tu clave privada (secreta)
    ↓
[Firma el mensaje con ECDSA]
    ↓
Firma (v, r, s) + Mensaje
    ↓
[Cualquiera puede verificar que fuiste TÚ]
    ↓
Se recupera tu dirección pública
```

### Ejemplo en una transacción EOA

```solidity
// Tu clave privada (¡NUNCA la compartas!)
privateKey = 0xab123...cd456

// Quieres enviar ETH, firmas la transacción
message = keccak256(rlp(transaction))
signature = ECDSA.sign(message, privateKey)  // Resultado: (v, r, s)

// La red verifica que fue realmente TÚ
recovered_address = ecrecover(message, v, r, s)
require(recovered_address == tx.from, "Firma inválida")
```

### Por qué se llama "Elliptic Curve"

Usa matemáticas de curva elíptica (`y² = x³ + ax + b`), lo que lo hace más seguro que RSA con claves mucho más pequeñas (256 bits en Ethereum vs 2048+ en RSA).

### ECDSA en EOA vs Smart Accounts

**EOA — siempre ECDSA, sin excepciones:**
```solidity
// El protocolo verifica esto directamente
ecrecover(txHash, v, r, s) == tx.from
```

**Smart Account — ECDSA es solo una opción:**
```solidity
function validateUserOp(...) {
    // Opción 1: ECDSA (como EOA)
    // Opción 2: Multi-sig (múltiples firmas)
    // Opción 3: WebAuthn (biométrico / passkey)
    // Opción 4: Cualquier lógica custom
}
```

Esta flexibilidad es una de las ventajas centrales de ERC-4337: la validación deja de estar hardcodeada en el protocolo y pasa a ser programable por el contrato.
